...
bool DS1302_getDateTime(DS1302_Dev* dev, DS1302_DateTime* dateTime)
{
    ...
    if ((dateTime->second > 59) ||
        ...
        (dateTime->year > 2099))
    {
        // printf("%d/%02d/%02d-%02d:%02d:%02d", dateTime->year, dateTime->month, dateTime->dayMonth, dateTime->hour, dateTime->minute, dateTime->second);
        // printf("\r\n");
...
void DS1302_writeByte(DS1302_Dev* dev, uint8_t value)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        gpio_set_level(dev->ioPin, value & 0x01);
        value >>= 1;
        gpio_set_level(dev->clkPin, 1);
        gpio_set_level(dev->clkPin, 0);
    }
